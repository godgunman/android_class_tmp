...
public class MainActivity extends Activity {
	...
	private final static String ROOT_URL = "http://graph.facebook.com/";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		button1.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				String userId = addressEditText.getText().toString();
				EXAMPLE_URL = ROOT_URL + userId;
	...
	private class UrlLoader extends AsyncTask<Integer, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (result != null) {
				...
				Gson gson = new Gson();
				FacebookInfoModel info = gson.fromJson(result,
						FacebookInfoModel.class);

				resultTextView.setText(String.format(
						"id:%s, name:%s, gender:%s, locale:%s", info.id,
						info.name, info.gender, info.locale));
			}
			progress.dismiss();
		}
	}
