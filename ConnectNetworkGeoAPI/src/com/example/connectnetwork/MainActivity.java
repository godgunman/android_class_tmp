...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		button1.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				String params = String.format("?address=%s&sensor=false",
						address);
	 ...
	 * 
	 * 另外要注意的是，任何會改變 UI 的操作一定要在 Main Thread 中執行。譬如 TextView 裡面的 setText() 由於
	 * doInBackground() 並不是 Main Thread 所以不能在這裡操作 UI。
	 */
	...
	private class UrlLoader extends AsyncTask<Integer, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (result != null) {
				...
				Gson gson = new Gson();
				GeoModel.geometry.location[] models = gson.fromJson(result,
						GeoModel.geometry.location[].class);

				// JSONObject object = new JSONObject(result);
				// JSONObject modelObject = object.getJSONArray("results")
				// .getJSONObject(0);
				// GeoModel.geometry.location model = gson.fromJson(
				// modelObject.toString(),
				// GeoModel.geometry.location.class);

				resultTextView.setText("lat=" + models[0].lat + ", lng="
						+ models[0].lng);
